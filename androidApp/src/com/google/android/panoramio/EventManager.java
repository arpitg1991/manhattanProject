 ...
 * maintained by a single instance of the EventManager class.
 */
public class EventManager {
   private static final String TAG = "Panoramio";
    ...
    /**
     * Holds the single instance of a EventManager that is shared by the process.
     */
     ...
     */
    private ArrayList<PanoramioEventItem> mImages = new ArrayList<PanoramioEventItem>();
    ...
    public static EventManager getInstance(Context c) {
        if (sInstance == null) {
            sInstance = new EventManager(c.getApplicationContext());
        }
        return sInstance;
    }
     ...
     */
    private void add(PanoramioEventItem item) {
        mImages.add(item);   
        notifyObservers();
    }
     ...
     */
    public PanoramioEventItem get(int position) {
        return mImages.get(position);
    }
    
    /**
     * Adds an observer to be notified when the set of items held by this EventManager changes.
     */
        ...
        private void parse() throws Exception {
            try {
                ...
                //int count = array.length();
            		...
            		List<PanoramioEventItem> names = dbInstance.getVideoForHomePage() ;	
            		int count = names.size();
            		
            		String chutiya = new Integer(count).toString();
            		System.out.println("chutiya " + chutiya) ;
            		
            		Log.d("hello",new Integer(count).toString()) ;
...
//                    Bitmap b = BitmapUtils.loadBitmap(thumb);
                    ...
                    final PanoramioEventItem item = new PanoramioEventItem(1,  "hello arpit");
                    if (item == null){ 
                    	Log.d("hello", "arpit") ; 
